...
- [ ] **Step 1:** 写失败测试 —— `ChatService.chat(String)` 经 mock ChatClient 返回期望文本；多轮 `chat(String, conversationId)` 注入 memory。
- [ ] **Step 2:** 定义 ChatService 端口（单轮/多轮/流式扩展点）。
- [ ] **Step 3:** 实现 ChatClientAdapter 适配 Spring AI ChatClient；可选实现 core `AiHandler`。
- [ ] **Step 4:** ChatProperties + AutoConfiguration。
- [ ] **Step 5:** 绿后 `git commit -m "feat(chat): 实现对话组件基础能力"`。
...
- [ ] **Step 1:** 写失败测试 —— add/get/clear 契约；窗口截断 N 轮；摘要触发阈值。
- [ ] **Step 2:** 定义 MemoryService 端口，适配 Spring AI `ChatMemory`。
- [ ] **Step 3:** 实现 InMemory（默认）/ Redis / Jdbc 多后端。
- [ ] **Step 4:** MemoryProperties（后端类型/窗口/阈值）+ 按后端条件装配。
- [ ] **Step 5:** Redis/Jdbc 用 Testcontainers 集成测试。
- [ ] **Step 6:** 绿后 `git commit -m "feat(memory): 实现会话记忆多后端组件"`。
...
- [ ] **Step 1:** 写失败测试 —— 单条/批量嵌入维度一致；批量分片不超批量上限；限流重试。
- [ ] **Step 2:** 定义 EmbeddingService 端口（`embed`/`embedBatch`）。
- [ ] **Step 3:** 实现 EmbeddingModelAdapter；metadata 透传模型名/维度。
- [ ] **Step 4:** 绿后 `git commit -m "feat(embedding): 实现向量嵌入组件"`。
...
- [ ] **Step 1:** 写失败测试 —— add/delete/search 契约；metadata 过滤；分块入库。
- [ ] **Step 2:** 定义 VectorDbService 端口。
- [ ] **Step 3:** 实现多后端 Adapter；分块策略可插拔（字符/Token/递归）。
- [ ] **Step 4:** 多后端用 Testcontainers 冒烟。
- [ ] **Step 5:** 绿后 `git commit -m "feat(vectordb): 实现向量库多后端组件"`。
...
- [ ] **Step 1:** 写联合冒烟：多轮对话经 memory 保持上下文；嵌入入库后可检索。
- [ ] **Step 2:** 绿后 `git commit -m "test(samples): LLM 基础层联合冒烟"`。
...
- [ ] **Step 1:** 每组件一个最小可运行示例（配置 + 注入 + 调用）。
- [ ] **Step 2:** `git commit -m "docs(samples): 补充 chat/memory/embedding/vectordb 示例"`。
